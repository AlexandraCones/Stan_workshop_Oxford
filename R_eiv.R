...
eiv_model_fit <- eiv_model$sample(
...
draws <- eiv_model_fit$draws()
parameters <- c("beta_0", "beta_0_w", "beta_1", "beta_1_w", "beta_2_w", "beta_3_w",
                "Var_alpha_0", "Var_alpha_1", "Cov_alpha_0_alpha_1", "sigma_0", "sigma_0_w")

# divergences, treedepth, ebfmi
eiv_model_fit$diagnostic_summary()

# rhat, ESS
eiv_model_fit$summary(parameters) |> select(variable, rhat, ess_bulk, ess_tail)
...
eiv_model_fit$summary(parameters)
...
# plot random effects
random_parameters <- c("Var_alpha_0", "Var_alpha_1", "Cov_alpha_0_alpha_1")
