...
dhglm_model_fit <- eiv_model$sample(
...
draws <- dhglm_model_fit$draws()
parameters <- c("beta_0", "beta_0_w", "beta_1_w", "beta_2_w", "beta_2_w", "beta_3_w",
                "Var_alpha_0", "Var_alpha_sigma", "Rho_alpha_0_sigma", "Cov_alpha_0_sigma")

# divergences, treedepth, ebfmi
dhglm_model_fit$diagnostic_summary()

# rhat, ESS
dhglm_model_fit$summary(parameters) |> select(variable, rhat, ess_bulk, ess_tail)
...
dhglm_model_fit$summary(parameters)

# ===================================== plot results ====================================================== #

# fixed effects
fixed_parameters <- c("beta_0", "beta_0_w", "beta_1_w", "beta_2_w")
fixed_draws <- dhglm_model_fit$draws(fixed_parameters, format = "df") %>%
  pivot_longer(cols = all_of(fixed_parameters), names_to = "variable", values_to = "value")
fixed_fit <- dhglm_model_fit$summary(fixed_parameters)
...
# plot random effects
random_parameters <- c("Var_alpha_0", "Var_alpha_sigma", "Rho_alpha_0_sigma", "Cov_alpha_0_sigma")
random_draws <- dhglm_model_fit$draws(random_parameters, format = "df") %>%
  pivot_longer(cols = all_of(random_parameters), names_to = "variable", values_to = "value")
random_fit <- dhglm_model_fit$summary(random_parameters)
