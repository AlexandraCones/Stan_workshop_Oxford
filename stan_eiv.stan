...
// ~~~ w = beta_0_w + beta_1_w*alpha_0_z[ID] + beta_2_w*alpha_1_z[ID] + beta_3_w*alpha_0_z[ID]*alpha_1_z[ID] + e_w ~~~ //
...
parameters {
  ...
  vector<lower=0>[2] sd_alpha;       // SDs for intercept, slope, fitness intercept
...
model {
  ...
  vector[N_w] mu_w = beta_0_w
                 ...
                 + beta_3_w * col(alpha,1)[ID_w] .* col(alpha,2)[ID_w]; // mean model fitness
